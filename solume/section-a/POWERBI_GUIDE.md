# Section A — Power BI Step-by-Step Guide
...
4. In the preview window click **Transform Data** (do NOT click Load yet — we need to clean first).
...
### Fix 1 — Clamp negative BEV_Share to 0

1. Select the `BEV_Share` column.
2. In the ribbon: **Add Column → Custom Column**.
3. Name it `BEV_Share_Clean`, formula:
   ```
   if [BEV_Share] < 0 then 0 else [BEV_Share]
   ```
4. Right-click the original `BEV_Share` column → **Remove**.
5. Right-click `BEV_Share_Clean` → **Rename** → `BEV_Share`.

### Fix 2 — Flag pre-launch i4/iX rows (before 2021)

1. **Add Column → Custom Column**, name `DataQualityFlag`, formula:
   ```
...
### Fix 3 — Correct column types

Ensure these types are set (click column header → **Data Type**):
...
## Step 3 — Create a Calculated Column (for Q B)
...
## Step 4 — Page 0: Data Quality

1. Right-click the page tab → **Rename** → `0 - Data Quality`.
2. Add a **Card** visual:
   - Field: count of rows where `DataQualityFlag = "Pre-launch noise"` — create a measure:
     ```
...
## Step 5 — Page 1: Question A (BEV & Electrification)

Add a new page, rename it `1 - Q A: Electrification`.

### Chart 1 — BEV_Share trend by Region
- Visual: **Line Chart**
- X-axis: `Year`
- Y-axis: `Average of BEV_Share`
- Legend: `Region`
- Title: "BEV Share Trend by Region (2018–2025)"

### Chart 2 — BEV_Share vs Units_Sold correlation
...
### Chart 3 — BEV_Share vs Revenue_EUR correlation
- Visual: **Line and Clustered Column Chart**
- X-axis: `Year`
...
### Chart 4 — Region comparison matrix
- Visual: **Matrix**
- Rows: `Region`
- Columns: `Year`
- Values: `Average of BEV_Share`
- Apply conditional formatting: **Home → Conditional Formatting → Background Color** (gradient low→high) to highlight strongest transitions.

### Chart 5 — KPI: Highest BEV_Share region in 2025
- Create a measure:
  ```
...
## Step 6 — Page 2: Question B (Price Elasticity)

Add a new page, rename it `2 - Q B: Price Elasticity`.

### Chart 1 — Scatter: Price vs Demand
- Visual: **Scatter Chart**
- X-axis: `Average of Avg_Price_EUR`
- Y-axis: `Sum of Units_Sold`
- Legend: `Model`
- Play Axis: `Year` (optional — shows evolution over time)
- Title: "Price vs Units Sold by Model (Elasticity Signal)"
- *Downward slope = negative elasticity (higher price → fewer units sold)*

### Chart 2 — Price elasticity by GDP bucket
...
### Chart 3 — Matrix: Model × GDP_Bucket
- Visual: **Matrix**
- Rows: `Model`
- Columns: `GDP_Bucket`
...
## Step 7 — Page 3: Question C (Seasonality)

Add a new page, rename it `3 - Q C: Seasonality`.

### Chart 1 — Revenue_EUR seasonal pattern
- Visual: **Line Chart**
- X-axis: `Month`
- Y-axis: `Average of Revenue_EUR`
- Legend: `Region`
- Title: "Avg Monthly Revenue EUR by Region"
- *Tip: collapse all years by NOT adding Year to the axis — Power BI averages across all years automatically.*

### Chart 2 — Units_Sold seasonal pattern
- Visual: **Line Chart**
- X-axis: `Month`
...
### Chart 3 — Economic indicators vs Revenue_EUR
- Visual: **Line and Clustered Column Chart**
- X-axis: `Month`
...
### Chart 4 — Economic indicators vs Units_Sold
...
1. **File → Save As** → `bmw_analysis.pbix`
...
- Keep `DataQualityFlag = "Clean"` filter active on all analysis pages (Pages 1–3) using the **Filters pane** → add `DataQualityFlag` as a page-level filter set to `"Clean"`. This excludes the 9 negative BEV and 288 pre-launch rows from analysis.
- Use **Format → Title** on each visual to add descriptive titles.
- Use **View → Themes** for a consistent colour scheme.
